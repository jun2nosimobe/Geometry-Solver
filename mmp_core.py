...
class Definition:
    ...
    def __hash__(self):
        # 🌟 FIX: リストやタプルが親にいた場合も安全にハッシュ化する
        def _get_hashable(p):
            if hasattr(p, 'id'): return p.id
            if hasattr(p, 'value'): return p.value
            if isinstance(p, (list, set)): return tuple(_get_hashable(item) for item in p)
            return p
            
        return hash((self.def_type, tuple(_get_hashable(p) for p in self.parents)))
...
class GeoEntity:
    ...
    def calculate(self, t_dict, cache):
        ...
        if def_type in CALCULATORS:
            try:
                # 🌟 FIX: 第1引数は parents ではなく self (自分自身) を渡す！
                result = CALCULATORS[def_type](self, t_dict, cache)
                if not result:
                    parent_errs = []
                    for p in parents:
...
def link_logical_incidence(entity1: GeoEntity, entity2: GeoEntity):
    # 🌟 FIX: ゴーストの密輸を完全に防ぐため、引数を必ず最新の代表元に変換する！
    rep1 = entity1.get_rep() if hasattr(entity1, 'get_rep') else entity1
    rep2 = entity2.get_rep() if hasattr(entity2, 'get_rep') else entity2
    
    c1 = rep1.get_best_component()
    c2 = rep2.get_best_component()
    
    # 代表元同士で双方向にリンクを張る
    if c1 and rep2 not in c1.subobjects:
        c1.subobjects.add(rep2)
    if c2 and rep1 not in c2.subobjects:
        c2.subobjects.add(rep1)
    ...
    # ==========================================
        ...
        if hasattr(env, 'merge_entities_logically'):
            ...
            for n in env.nodes:
                ...
                if getattr(n.get_rep(), 'entity_type', '') == "Scalar":
                    nc = n.get_rep().get_best_component()
            ...
            ratio_name = f"Ratio_{getattr(A, 'name', 'A')}{getattr(M, 'name', 'M')}_{getattr(M, 'name', 'M')}{getattr(B, 'name', 'B')}_(Auto)"
            ratio_ent = None
            for n in env.nodes:
                if not getattr(n, 'is_valid', lambda: True)(): continue
                if getattr(n.get_rep(), 'entity_type', '') == "Scalar":
                    nc = n.get_rep().get_best_component()
                    if nc and any(d.def_type == "AffineRatio" and [p.get_rep() if hasattr(p, 'get_rep') else p for p in d.parents] == [A.get_rep(), M.get_rep(), B.get_rep()] for d in nc.definitions):
                        ratio_ent = n.get_rep()
                        break
            if not ratio_ent:
...
def create_geo_entity(def_type: str, parents: List[Any], name: str = "", env=None, is_given=False, is_ghost=False, importance: float = None) -> 'GeoEntity':
    ...
    # ==========================================
    # 🌟 FIX: ゴースト判定とメタデータ計算を、Hash Consingの前に移動！
    # ==========================================
    # 3. ゴースト判定
    ...
    # ==========================================
    if env is not None and not is_true_ghost and def_type not in ["Point", "Given", "Free", "Constant", "GivenPoint", "FreePoint"]:
        rep_parents = tuple(p.get_rep() if hasattr(p, 'get_rep') else p for p in valid_parents)
        ...
        for existing_node in env.nodes:
            if not getattr(existing_node, 'is_valid', lambda: True)(): continue
            ...
            for d in comp.definitions:
                if d.def_type == def_type and len(d.parents) == len(rep_parents):
                    # 既存図形の親を最新にして比較
                    existing_d_parents = tuple(p.get_rep() if hasattr(p, 'get_rep') else p for p in d.parents)
                    if def_type in unordered_types:
                    ...
                    if target_signature == (def_type, existing_d_parents):
                        ...
                        logger.debug(f"    ♻️ [メモ化再利用] 新規作成をスキップし既存図形 {existing_rep.name} を流用します: {def_type}({', '.join(getattr(p, 'name', str(p)) for p in rep_parents)})")
                        
                        # 🌟 ここで先ほど上で計算した naive_degree と depth を安全に使える！
                        new_def = Definition(def_type, list(parents), naive_degree, depth)
                        if new_def not in comp.definitions:
                            comp.definitions.add(new_def)
                            for p in valid_parents:
                                link_logical_incidence(p, existing_rep)
