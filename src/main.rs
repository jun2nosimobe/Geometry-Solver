...
fn main() {
    ...
    // 🌟 UCB1バンディットは既定で無効。--bandit で有効にできる。
    //
    // 🐛 --trace(§05-#2)の追跡で、発火した定理の平均優先度がどれも +5.0 付近に
    // 張り付いていることが分かった。theorem_priority_bonus が
    // clamp(score*5, -5, 5) だったため、ucb1_score が 1.0 を超えるだけで上限に
    // なり、学習結果が並べ替えに一切反映されていなかった。
    //
    // 🌟 そこで全37問の消費仕事量(壁時計ではないので機械に左右されない)を
    // 両モードで合計して測り直した:
    //   バンディット有効(飽和したまま) 8,088,733
    //   飽和を解いた版(尺度を50倍に)   7,954,186   ← 改善は1.7%どまり
    //   無効                           6,740,492   ← 20%少ない
    // solved はどれも 29/37 で同じ。しかも有効時に消費が変わる問題は37問中
    // 4問だけで(残り33問は1ステップも変わらない=並べ替えが起きていない)、
    // その4問すべてが悪化する側だった(bench_2012egmop1 が 682,107 →
    // 1,914,519 と2.8倍で最悪)。
    //
    // 飽和を解いてもほとんど改善しないということは、問題は尺度ではなく
    // 報酬の定義そのものにある。報酬は「このタスクが結論を適用できたか」で
    // 測っているが、--trace が示したのは「発火しても証明に残らない方が普通」
    // (証明に残る発火は全体の1〜10%)ということなので、この報酬は
    // 「役に立つ定理か」の代理指標として機能していない。尺度の修正
    // (prover.rs::theorem_priority_bonus)は残してあるので、報酬を作り直す
    // 実験は --bandit を付けるだけで再開できる。
    let bandit_enabled = args.iter().any(|a| a == "--bandit");
