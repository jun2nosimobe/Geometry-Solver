...
//!
//! 実装方針: 主系のModInt(mod 998244353、mmp_math.rs)とは完全に独立した
//! 新しい環Z/P^4Zを、基数Pのリトルエンディアン4桁のビッグ整数として実装
//! する(discover_viz.rs::RealEvaluatorが証明本体のModInt評価器と完全に
//! 独立な実数版評価器を新設したのと同じ考え方: 目的が違う数値実験は
//! 既存の健全性重視の設計に無理に相乗りさせず、素直に独立させる)。
//! Z/P^4Zは「基数Pの4桁ビッグ整数」そのものであり、繰り上がり/繰り下がり
//! を素直に実装するだけで良い。桁ごとの値はP<2^30程度なので、桁同士の
//! 積(<P^2<2^60)はi64/u128で全く問題なくオーバーフローしない。
//!
//! P(998244353よりわずかに小さい素数998244341)を主系のModInt::PRIME
//! (998244353)とは別に用意したのは、(a)混同を避けるため、(b)最終的な
//! 「mod pで一致するか」の判定がSchwartz-Zippel的な偶然一致率
//! (だいたい1/P)を保つために、Pが十分大きい(主系と同程度の桁数の)素数で
//! あってほしいため。

use rand::Rng;

/// Z/P^PRECZ の基数(桁の底)。主系のModInt::PRIMEとは別の、それより
/// わずかに小さい素数(混同回避)。
pub const P: i64 = 998244341;
